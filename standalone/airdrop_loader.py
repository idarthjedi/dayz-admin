...
def main(
    expansion_config_dir: str,
    primary_search_fragment: str,
    market_search_file: str,
    primary_search_section: str,
):
    """
    ...
    FileManager.backup(
        os.path.join(expansion_config_dir, "Settings", "AirdropSettings.json")
    )

    # load airdropsettings file
    with open(
        os.path.join(expansion_config_dir, "Settings", "AirdropSettings.json"), "r"
    ) as airdropsettings_file:
        airdrop_data = json.load(airdropsettings_file)

    classname_search = json_parser.parse(
        f"$..Containers[?(@.Container == '{primary_search_section}')]"
    ...
    classname_search = json_parser.parse(
        f"$..Items[?(@.ClassName =~ '{primary_search_fragment}')]"
    )
    ...
    variants = []
    for vars in objs[1::]:
        variants.append({"Name": f"{vars}", "Attachments": [], "Chance": chance})
    ...
    with open(
        os.path.join(expansion_config_dir, "Settings", "AirdropSettings.json"), "w"
    ) as airdropsettings_file:
