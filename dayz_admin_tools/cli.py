...
def main() -> None:
    ...
    p_validate = subparsers.add_parser(
        "validate", help="Validate JSON and/or XML files"
    )
    p_validate.add_argument(
        "-t", "--type", choices=["JSON", "XML", "BOTH"], default="BOTH"
    )
    p_validate.add_argument(
        "-d", "--dir", action="append", required=True, help="Directory to validate"
    )
    ...
    p_items.add_argument(
        "-d", "--dir", required=True, help="Expansion Market directory"
    )
    ...
    p_airdrop.add_argument(
        "-d", "--dir", required=True, help="Expansion config directory"
    )
    p_airdrop.add_argument(
        "-i", "--item", required=True, help="Item fragment to search for"
    )
    ...
    p_vp = convert_sub.add_parser(
        "vehicle-parts", help="Convert TraderPlus vehicle parts to Expansion"
    )
    p_vp.add_argument(
        "-f", "--file", required=True, help="TraderPlus vehicle parts file"
    )
